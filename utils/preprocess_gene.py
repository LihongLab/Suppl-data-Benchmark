...
def ensp_to_hugo_map():
    with open('/home/shenbihan/DLbenchmark/DL_models/TGSA/data/9606.protein.info.v11.5.txt') as csv_file:
...
def hugo_to_ncbi_map():
    with open('/home/shenbihan/DLbenchmark/DL_models/TGSA/data/enterez_NCBI_to_hugo_gene_symbol_march_2019.txt') as csv_file: 
        next(csv_file)  # Skip first line
        csv_reader = csv.reader(csv_file, delimiter='\t')
...
def save_cell_graph_all(exp_path, cn_path, mu_path, save_path):
#     if not os.path.exists(save_path):
#         os.makedirs(save_path)
    exp = pd.read_csv(exp_path, index_col=0)
    cn = pd.read_csv(cn_path, index_col=0)
    mu = pd.read_csv(mu_path, index_col=0)
    # me = pd.read_csv(os.path.join(genes_path, 'me.csv'), index_col=0)
    # print('Miss values：{}，{}，{}, {}'.format(exp.isna().sum().sum(), cn.isna().sum().sum(), mu.isna().sum().sum(),
    #                                         me.isna().sum().sum()))
...
#     cn = scaler.fit_transform(cn)
    ...
    np.save(save_path, cell_dict)
    print("finish saving cell exp + cn + mu data!")
    
def save_cell_graph_exp(genes_path, save_path):
#     if not os.path.exists(save_path):
#         os.makedirs(save_path)
    exp = pd.read_csv(genes_path, index_col=0)
    # cn = pd.read_csv(os.path.join(genes_path, 'cn.csv'), index_col=0)
    # mu = pd.read_csv(os.path.join(genes_path, 'mu.csv'), index_col=0)
    # me = pd.read_csv(os.path.join(genes_path, 'me.csv'), index_col=0)
    # print('Miss values：{}，{}，{}, {}'.format(exp.isna().sum().sum(), cn.isna().sum().sum(), mu.isna().sum().sum(),
    ...
    exp = pd.DataFrame(exp, index=index, columns=columns)
    # cn = pd.DataFrame(cn, index=index, columns=columns)cn
    # mu = pd.DataFrame(mu, index=index, columns=columns)
    # me = pd.DataFrame(me, index=index, columns=columns)
    cell_names = exp.index
    # print('Miss values：{}，{}，{}, {}'.format(exp.isna().sum().sum(), cn.isna().sum().sum(), mu.isna().sum().sum(),
    ...
    np.save(save_path, cell_dict)
    print("finish saving cell exp data!")
    
def save_cell_graph_cnv(genes_path, save_path):
#     if not os.path.exists(save_path):
#         os.makedirs(save_path)
#     exp = pd.read_csv(os.path.join(genes_path, 'exp.csv'), index_col=0)
    cn = pd.read_csv(genes_path, index_col=0)
    # mu = pd.read_csv(os.path.join(genes_path, 'mu.csv'), index_col=0)
    # me = pd.read_csv(os.path.join(genes_path, 'me.csv'), index_col=0)
    # print('Miss values：{}，{}，{}, {}'.format(exp.isna().sum().sum(), cn.isna().sum().sum(), mu.isna().sum().sum(),
    #                                         me.isna().sum().sum()))
...
#         cell_dict[i] = Data(x=torch.tensor([exp.loc[i]], dtype=torch.float).T)
    ...
    np.save(save_path, cell_dict)
    print("finish saving cell cnv data!")

def save_cell_graph_mut(genes_path, save_path):
#     if not os.path.exists(save_path):
#         os.makedirs(save_path)
#     exp = pd.read_csv(os.path.join(genes_path, 'exp.csv'), index_col=0)
    # cn = pd.read_csv(os.path.join(genes_path, 'cn.csv'), index_col=0)
    mu = pd.read_csv(genes_path, index_col=0)
    # me = pd.read_csv(os.path.join(genes_path, 'me.csv'), index_col=0)
    # print('Miss values：{}，{}，{}, {}'.format(exp.isna().sum().sum(), cn.isna().sum().sum(), mu.isna().sum().sum(),
    #                                         me.isna().sum().sum()))
...
def get_STRING_graph(genes_path, thresh=0.95, prefix = ""):
    save_path = os.path.join('/home/shenbihan/DLbenchmark/DL_models/TGSA/data/CellLines_DepMap/CCLE_580_18281/census_706', 
                             prefix + 'edge_index_PPI_{}.npy'.format(thresh)
                            )
...
#         edges = pd.read_csv('./data/9606.protein.links.detailed.v11.0.txt', sep=' ')
        edges = pd.read_csv('/home/shenbihan/DLbenchmark/DL_models/TGSA/data/9606.protein.links.v11.5.txt', sep=' ')
        ...
        # 既在706个基因里面，也在PPI网络里面的基因才挑出来进行连边，呵呵，等会核对一下这300+个基因的名字
        for i in edge_list:
        ...
        np.save(
            os.path.join('/home/shenbihan/DLbenchmark/DL_models/TGSA/data/CellLines_DepMap/CCLE_580_18281/census_706', 
                         prefix + 'edge_index_PPI_{}.npy'.format(thresh)),
            edge_index)
    else:
