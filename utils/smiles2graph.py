...
def save_drug_graph(smiles_path, output_path):
    smiles = pd.read_csv(smiles_path, sep = '\t')
    drug_dict = {}
    for i in range(len(smiles)):
        drug_dict[smiles.iloc[i,smiles.columns.get_loc('name')]] = smiles2graph(smiles.iloc[i, smiles.columns.get_loc('CanonicalSMILESrdkit')])
    np.save(os.path.join(output_path,'drug_feature_graph.npy'), drug_dict)
    return drug_dict
